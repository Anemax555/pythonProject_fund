...
def input_yield_mysql(params):
    ...
    for i in range(len(params)):
        ...
        if (params[i] != ''):
            if (i > 2):
                tmp = (float(params[i]),)
                params_sql = params_sql + tmp
            else:
                tmp = (params[i],)
                params_sql = params_sql + tmp
            replace_sql.append(rate_type[i])
            values_sql.append('%s')

    replace_sql = ','.join(replace_sql)
    values_sql = ','.join(values_sql)
    sql = f'replace into f_fund_yield ({replace_sql}) values ({values_sql})'
    # sql = 'insert into f_fund_yield (f_fund_id,f_manager_id) values (%s,%s)'
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(len(fund_open_fund_rank_em_df['基金代码']))):
        params = (
            fund_open_fund_rank_em_df['基金代码'][i],
            fund_open_fund_rank_em_df['基金简称'][i],
            fund_open_fund_rank_em_df['日期'][i],
            fund_open_fund_rank_em_df['日增长率'][i],
            fund_open_fund_rank_em_df['近1周'][i],
            fund_open_fund_rank_em_df['近1月'][i],
            fund_open_fund_rank_em_df['近3月'][i],
            fund_open_fund_rank_em_df['近6月'][i],
            fund_open_fund_rank_em_df['近1年'][i],
            fund_open_fund_rank_em_df['近2年'][i],
            fund_open_fund_rank_em_df['近3年'][i],
            fund_open_fund_rank_em_df['今年来'][i],
            fund_open_fund_rank_em_df['成立来'][i]
        )
        # print(params)
        input_yield_mysql(params)
